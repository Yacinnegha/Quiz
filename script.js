...
/* التحقق من صلاحية التقدم المحفوظ — lim = حد عدد الأسئلة الحالي في الواجهة */
function validateProgress(s, lim) {
  const p = s && s.progress;
  if (!p || typeof p !== "object") return null;
  const full = Array.isArray(s.analysis && s.analysis.paragraphs) ? s.analysis.paragraphs.length : 0;
  if (!full) return null;
  const expected = lim > 0 ? Math.min(lim, full) : full;
  if (p.activeCount !== expected) return null; /* تغيّر حد الأسئلة أو التحليل منذ الحفظ */
...
function renderResume() {
  ...
  const prog = validateProgress(s, parseInt(el("question-limit").value, 10) || 0);
...
function buildPrintSheet() {
  ...
  html += "</ol></section>";
  html += '<footer class="ps-foot">أُنشئ بواسطة «اختبرني» — الإصدار 1.9</footer>';
