...
/* قوالب سؤال المحاكاة حسب لغة السؤال المختارة — النص المقنّع يبقى بلغة النص الأصلي (محاكاة محلية بلا ترجمة فعلية) */
const DEMO_LANG_TPL = {
  ar: { q: "أكمل الفراغ بالكلمة الصحيحة المناسبة:", exp: (k, t) => `الكلمة «${k}» وردت في الفقرة ضمن الجملة: «${t}»` },
  en: { q: "Fill in the blank with the correct word:", exp: (k, t) => `The word «${k}» appears in the paragraph in the sentence: «${t}»` },
  fr: { q: "Complétez le vide avec le mot correct :", exp: (k, t) => `Le mot «${k}» apparaît dans le paragraphe dans la phrase : «${t}»` },
  es: { q: "Completa el espacio con la palabra correcta:", exp: (k, t) => `La palabra «${k}» aparece en el párrafo en la frase: «${t}»` },
  de: { q: "Ergänzen Sie die Lücke mit dem richtigen Wort:", exp: (k, t) => `Das Wort «${k}» kommt im Absatz im Satz vor: «${t}»` },
  tr: { q: "Boşluğu doğru kelimeyle tamamlayın:", exp: (k, t) => `«${k}» kelimesi paragrafta şu cümlede geçiyor: «${t}»` },
  ur: { q: "خالی جگہ صحیح لفظ سے پورا کریں:", exp: (k, t) => `لفظ «${k}» پیراگراف میں اس جملے میں آیا: «${t}»` },
  id: { q: "Lengkapi bagian kosong dengan kata yang tepat:", exp: (k, t) => `Kata «${k}» muncul dalam paragraf pada kalimat: «${t}»` },
  ru: { q: "Заполните пропуск подходящим словом:", exp: (k, t) => `Слово «${k}» встречается в абзаце в предложении: «${t}»` },
  zh: { q: "请用正确的词填空：", exp: (k, t) => `词语「${k}」出现在段落中的这个句子里：「${t}」` },
};
...
function demoGenerateQuestion({ paragraph, keyword, pool, difficulty: lvl }) {
  ...
  const tpl = DEMO_LANG_TPL[quizLang] || DEMO_LANG_TPL.ar;
  return {
    question: `${tpl.q}
«${masked}»`,
    options,
    correctIndex: options.indexOf(keyword),
    explanation: tpl.exp(keyword, shortTarget),
  };
}
...
/* ====== لغة السؤال (auto = لغة النص نفسها) ====== */
const LANG_NAMES = {
  auto: "لغة النص تلقائيًا",
  ar: "العربية",
  en: "الإنجليزية",
  fr: "الفرنسية",
  es: "الإسبانية",
...
function loadQuizLang() {
  const v = store.get("quiz_lang");
  quizLang = LANG_NAMES[v] ? v : "auto";
}

function getQuizLang() { return quizLang; }

function syncQuizLangUI() {
  el("quiz-lang").value = quizLang;
}

function setQuizLang(v, silent) {
  if (!LANG_NAMES[v]) return;
  quizLang = v;
  store.set("quiz_lang", v);
  syncQuizLangUI();
  if (!silent) {
    toast("info", "لغة السؤال: " + (v === "auto" ? "تلقائي — بلغة النص." : LANG_NAMES[v] + " — سيولّد السؤال والخيارات والتعليل بها."));
  }
}

/* تعليمة اللغة تُدرج في برومبت توليد السؤال عند اختيار لغة محددة */
function buildLangDirective() {
  if (quizLang === "auto") return "";
  return "مهم — لغة السؤال: اكتب السؤال والخيارات الأربعة والتعليل كلها باللغة " + LANG_NAMES[quizLang] +
    ". وإذا كانت الفقرة أو الكلمة المفتاحية بلغة أخرى فترجم المعنى بدقة دون تغيير المضمون، ويُستحسن إثبات الكلمة المفتاحية الأصلية بين قوسين عند أول ذكر لها.";
}
...
function renderQuestion(q) {
  ...
  q.options.forEach((opt, idx) => {
    ...
    text.dir = "auto"; /* عرض صحيح لخيارات اللغات الأجنبية */
...
/* حالة التنقل داخل نافذة الفقرة — idx الفقرة المعروضة، max أقصى فقرة مسموحة (فقرة السؤال الذي فُتحت منه النافذة) */
let passageNavIdx = -1;
let passageNavMax = -1;
...
function openPassageModal(q) {
  ...
  /* إحصاء: كم مرة استعان المستخدم بالفقرات في هذه الجولة (يُحتسب عند الفتح لا مع كل تنقل) */
  if (state.quiz) state.quiz.passagesViewed = (state.quiz.passagesViewed || 0) + 1;

  /* التنقل متاح فقط لفقرات التحليل الفعلية (لا الاختبارات المستوردة) وعندما توجد أكثر من فقرة —
     والحد الأقصى هو فقرة السؤال الذي فُتحت منه النافذة حتى لا تُكشف فقرات الأسئلة القادمة */
  const navBase = (!state.staticQuestions && q && Number.isInteger(q.paragraphIndex) &&
    state.activeParagraphs[q.paragraphIndex] && state.activeParagraphs.length > 1) ? q.paragraphIndex : -1;
  passageNavIdx = passageNavMax = navBase;
  el("passage-nav").classList.toggle("hidden", navBase < 0);
  el("passage-hint").innerHTML = navBase >= 0
    ? "تنقّل بين فقرات الأسئلة السابقة والحالية فقط — <kbd>Esc</kbd> للإغلاق"
    : "اقرأ الفقرة بتمعّن ثم أكمل — <kbd>Esc</kbd> للإغلاق";

  if (navBase >= 0) {
    renderPassageAt(navBase, false, q);
  } else {
    renderPassageStatic(passage, q);
  }
...
/* عرض فقرة ثابتة (سياق عام للاختبارات المستوردة أو فقرة وحيدة) */
function renderPassageStatic(passage, q) {
  ...
  body.innerHTML = "";
  body.appendChild(highlightKeywordFrag(passage.text, q && q.keyword));
  body.scrollTop = 0;
}

/* عرض فقرة بمؤشر معين مع تحديث العنوان والكلمة المفتاحية والعدّاد وأزرار التنقل */
function renderPassageAt(idx, animate, baseQ) {
  const p = state.activeParagraphs[idx];
  if (!p || !p.text) return false;
  const navQ = (baseQ && baseQ.paragraphIndex === idx && baseQ.keyword)
    ? baseQ
    : ((state.questions && state.questions.get(idx)) || null);
  const kw = (navQ && navQ.keyword) || (Array.isArray(p.keywords) && p.keywords[0]) || "";
  el("passage-title-text").textContent = "الفقرة المتعلقة بالسؤال " + (idx + 1);
  const kwWrap = el("passage-kw");
  kwWrap.innerHTML = "";
  if (kw) {
    const chip = document.createElement("span");
  ...
  const body = el("passage-body");
  body.innerHTML = "";
  body.appendChild(highlightKeywordFrag(p.text, kw));
  body.scrollTop = 0;
  if (animate) {
    body.classList.remove("nav-anim");
    void body.offsetWidth; /* إعادة تشغيل الأنيميشن */
    body.classList.add("nav-anim");
  }
  el("passage-nav-pos").textContent = "السؤال " + (idx + 1) + " من " + (passageNavMax + 1);
  el("btn-passage-prev").disabled = idx <= 0;
  el("btn-passage-next").disabled = idx >= passageNavMax;
  return true;
}

/* التنقل: dir = -1 للسابق (فقرات أقدم) و+1 للتالي — لا يتجاوز فقرة السؤال الذي فُتحت منه النافذة مطلقًا */
function passageGo(dir) {
  if (passageNavIdx < 0) return;
  const target = passageNavIdx + dir;
  if (target < 0 || target > passageNavMax) return;
  passageNavIdx = target;
  renderPassageAt(passageNavIdx, true);
}
...
function buildPrintSheet() {
  ...
  html += "</ol></section>";
  html += '<footer class="ps-foot">أُنشئ بواسطة «اختبرني» — الإصدار 1.11</footer>';
...
function onKeydown(e) {
  ...
  if (isPassageOpen()) {
    if (e.key === "Escape") {
      ...
      /* RTL: اليمين = الفقرة السابقة (سؤال أقدم) */
      e.preventDefault();
      passageGo(-1);
    } else if (e.key === "ArrowLeft") {
      /* RTL: اليسار = الفقرة التالية (بحد السؤال الحالي) */
      e.preventDefault();
      passageGo(1);
    }
    return;
...
function init() {
  ...
  loadQuizLang();
  syncQuizLangUI();
  el("quiz-lang").addEventListener("change", () => setQuizLang(el("quiz-lang").value));
  ...
  el("btn-passage-prev").addEventListener("click", () => passageGo(-1));
  el("btn-passage-next").addEventListener("click", () => passageGo(1));
